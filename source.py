...
def http_string_convertor(query):
    final = urllib.parse.quote_plus(query)
    return final
...
##    ops_query = int(input_query * 100000)
    binary = bin_convertor(int(ops_query))
...
def bin_convertor(query):
    if query >= 0:
        final = bin(query) [2:]
        for num in list(range(8 - (len(final) % 8)))[::-1]:
            final = '0' + final
##        for num in range(4 - len(final)//8):
